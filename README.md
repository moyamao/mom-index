...
- 具体情绪标签：焦虑、愤怒、兴奋、乐观、悲观、失望、后悔、嘲讽、平静；单帖可有多个，仅计数，不参与市场情绪指数。
- 交易意图：`buy`（买入）、`add`（加仓）、`hold`（持有）、`reduce`（减仓）、`sell`（卖出）、`clear`（清仓）、`neutral`（观望）。
...
仓位比例也只在已识别仓位的样本中计算。`unknown` 不进入比例分母，页面同时展示有效样本数，避免少量样本产生误导。最终结果分别展示情绪、持仓状态和交易意图的帖子计数；资讯与垃圾内容不进入这三组统计。
